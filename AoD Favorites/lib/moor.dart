...
import 'package:Video/connection.dart';
import 'package:Video/consts.dart';
...
class Database extends _$Database {
    ...
    try {
      ...
      List<FavoritesCompanion> toInsert = data
          .map((e) => FavoritesCompanion.insert(
                ...
                epNum: e["epNum"],
                fsk: e["fsk"],
                marked: Value(e.containsKey("marked") && e["marked"] is int ? e["marked"] : 0),
                watchDate: e.containsKey("watchDate") ? Value(DateConverter.format.parse(e["watchDate"])) : Value.absent(),
                prodYear: e["prodYear"],
  ...
  Future updateData() async {
    List data = await select(favorites).get();
    ...
    data.removeWhere((f) => toRemove.contains(f.id));
    delete(favorites);
    await batch((batch) =>
